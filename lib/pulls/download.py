...
def pr_metadata(repo_name, num_pages):
    ...
    create_directory_structure(os.path.join('pulls', repo_name))
    with open(os.path.join('pulls', repo_name, 'pr_metadata.json'), 'w') as metadata_file:
...
def pr_files(repo_name):
    ...
    with open(os.path.join('pulls', repo_name, 'pr_files.json'), 'w') as metadata_file:
        json.dump(metadata_dict, metadata_file)
...
def pr_diffs(repo_name):
    ...
    with open(os.path.join('pulls', repo_name, 'pr_diffs.json'), 'w') as metadata_file:
