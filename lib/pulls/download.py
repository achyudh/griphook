...
def pr_metadata(repo_name, num_pages):
    ...
    create_directory_structure(os.path.join('data', 'pulls', repo_name))
    with open(os.path.join('data', 'pulls', repo_name, 'pr_metadata.json'), 'w') as metadata_file:
...
def pr_files(repo_name):
    ...
    :param repo_name: string in owner/repo_name format
    """
    with open(os.path.join('data', 'pulls', repo_name, 'pr_metadata.json'), 'r') as metadata_file:
    ...
    with open(os.path.join('data', 'pulls', repo_name, 'pr_files.json'), 'w') as metadata_file:
        json.dump(metadata_dict, metadata_file)
...
def pr_diffs(repo_name):
    ...
    diffs_dict = dict()
    for pull_request in tqdm(metadata_dict):
        diffs_dict[pull_request['number']] = github.diff(pull_request['diff_url'])

    with open(os.path.join('data', 'pulls', repo_name, 'pr_diffs.json'), 'w') as metadata_file:
